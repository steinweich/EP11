...
void new_word() {
	...
	if(state_machine_state > 1) {
		...
		// 20m cycles less
		// 120m instructions less
		// 
		// removing if (hashfunc > 0) and putting it into an "else-if" branch
		if (hashfunc == 1) {
			//methodone++;
			//6208419
			// printf("ID\n");
...
unsigned long next_state_machine(unsigned char *current_char) {
	...
	// 60m cycles less,
	// 20m instructions less,
	// 20k L1-dcache-load-misses less,
	// 400k branch misses less,
	//
	// changing if and else-if order
	if(next_class > 11) { // Kein naechster Status
