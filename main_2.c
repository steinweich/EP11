...
int next_state_machine_state = 0;
...
void search_next_state(int current_state, char *current_char) {
	
	next_state_machine_state = -1;
	...
	for(int i=0;;i++) {
		...
		if(char_class > 122) {
			if(char_class == '{' &&
				...
				next_state_machine_state = check_class;
				break;
			} else if(char_class == '}' &&
				(
...
void append_char_to_yytext(char *c) {
...
unsigned long next_state_machine(char *current_char) {
	...
	search_next_state(state_machine_state, current_char);
	
	if(next_state_machine_state == -1) { // Kein naechster status gefunden - wahrscheinlich neues wort
		
		if(yylen > 0) {
			accept_word();
		}
		
		search_next_state(0, current_char); // Probiere es vom start-zustand nochmals
		
		// Verlaengere Wort (bzw. fange neu an)		
		append_char_to_yytext(current_char);
		
		if(next_state_machine_state == -1) { // Ungueltiger character
		...
		if(next_state_machine_state == 0) { // Neues Wort wurde angefangen
			if(yylen > 0) {
				accept_word();
			}
	...
	state_machine_state = next_state_machine_state;
}
